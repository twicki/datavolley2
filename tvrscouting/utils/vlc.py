    ...
    if sys.platform.startswith("linux"):
        ...
        if p is None:
            ...
            if plugin_path is None:
                ...
                for p in ("Program Files\\VideoLan\\", "VideoLan\\", "Program Files\\", ""):
...
class CallbackDecorators(object):
    ...
    LogCb = ctypes.CFUNCTYPE(
        ctypes.c_void_p, ctypes.c_void_p, ctypes.c_int, Log_ptr, ctypes.c_char_p, ctypes.c_void_p
    )
...
class Instance(_Ctype):
    ...
    def vlm_add_broadcast(
        self, psz_name, psz_input, psz_output, i_options, ppsz_options, b_enabled, b_loop
    ):
...
class MediaPlayer(_Ctype):
    ...
    def get_full_title_descriptions(self):
        ...
        info = ctypes.cast(
            ctypes.titleDescription_pp, ctypes.POINTER(ctypes.POINTER(TitleDescription) * n)
        )
        return info
    ...
    def get_full_chapter_descriptions(self, i_chapters_of_title):
        ...
        info = ctypes.cast(
            ctypes.chapterDescription_pp, ctypes.POINTER(ctypes.POINTER(ChapterDescription) * n)
        )
        return info
...
def libvlc_audio_output_device_get(mp):
    ...
    f = _Cfunctions.get("libvlc_audio_output_device_get", None) or _Cfunction(
        "libvlc_audio_output_device_get", ((1,),), None, ctypes.c_char_p, MediaPlayer
    )
    return f(mp)
...
if __name__ == "__main__":
    ...
    if sys.argv[1:] and sys.argv[1] not in ("-h", "--help"):
        ...
        try:
            ...
            print(
                "NameError: %s (%s vs LibVLC %s)"
                % (sys.exc_info()[1], __version__, libvlc_get_version())
            )
            sys.exit(1)
